...
import { matchesAllowedUser, parseAllowedUsers, isOwnerSender, getSenderMemberInfo } from './allowlist.js';
...
const HERMES_HOME = process.env.HERMES_HOME || path.dirname(SESSION_DIR);
...
const LISTEN_KEYWORD = String(process.env.WHATSAPP_LISTEN_KEYWORD || 'ahwal').trim().toLowerCase();
const LISTEN_WEBHOOK_URL = String(process.env.WHATSAPP_LISTEN_WEBHOOK_URL || 'https://webhook.darsya.my.id/').trim();
// Opt-in, receive-only Team Manager capture. This deliberately records only
// inbound events and has no send capability; omit the profile ID to disable it.
const TEAM_MANAGER_OBSERVER = createTeamManagerObserver({
  profileId: String(process.env.WHATSAPP_TEAM_MANAGER_PROFILE_ID || '').trim(),
  rootDir: process.env.HERMES_HOME || path.dirname(SESSION_DIR),
});
const recentlyForwardedWebhookIds = createOutboundIdTracker(4096);
const webhookInFlight = new Set();
const webhookDb = new DatabaseSync(path.join(process.env.HERMES_HOME || path.dirname(SESSION_DIR), 'keyword_webhooks.sqlite'));
webhookDb.exec(`CREATE TABLE IF NOT EXISTS forwarded_messages (message_id TEXT PRIMARY KEY, forwarded_at INTEGER NOT NULL)`);
webhookDb.exec(`DELETE FROM forwarded_messages WHERE forwarded_at < unixepoch() - 2592000`);
webhookDb.exec(`DELETE FROM forwarded_messages WHERE rowid NOT IN (SELECT rowid FROM forwarded_messages ORDER BY forwarded_at DESC LIMIT 10000)`);
const webhookSeen = webhookDb.prepare('SELECT 1 FROM forwarded_messages WHERE message_id = ?');
const webhookMark = webhookDb.prepare('INSERT OR IGNORE INTO forwarded_messages (message_id, forwarded_at) VALUES (?, unixepoch())');

async function forwardKeywordMessage({ msg, chatId, senderId, body }) {
  const messageId = msg.key?.id;
  if (!LISTEN_KEYWORD || !LISTEN_WEBHOOK_URL || !messageId || !body.toLowerCase().includes(LISTEN_KEYWORD)) return;
  if (recentlyForwardedWebhookIds.has(messageId) || webhookSeen.get(messageId) || webhookInFlight.has(messageId)) return;
  webhookInFlight.add(messageId);
  const senderBareId = String(senderId).split('@', 1)[0].split(':', 1)[0];
  const senderPhone = senderBareId.replace(/\D/g, '') === senderBareId
    ? (lidToPhone[senderBareId] || senderBareId)
    : (lidToPhone[senderBareId] || null);
  const payload = {
    chatId,
    senderId,
    senderPhone,
  ...
  try {
    const response = await fetch(LISTEN_WEBHOOK_URL, {
      method: 'POST',
    ...
    webhookMark.run(messageId);
    recentlyForwardedWebhookIds.remember(messageId);
    console.log(JSON.stringify({ event: 'keyword_webhook_sent', chatId: redactWhatsAppId(chatId), messageId }));
  } catch (error) {
    console.warn(`[bridge] keyword webhook failed: ${error.message}`);
  } finally {
...
async function startSocket() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      const earlyBody = earlyContent.conversation || earlyContent.extendedTextMessage?.text || earlyContent.imageMessage?.caption || earlyContent.videoMessage?.caption || earlyContent.documentMessage?.caption || '';
      if (!msg.key.fromMe) {
        await forwardKeywordMessage({ msg, chatId, senderId, body: earlyBody });
      }
      emitDebugEvent({
        stage: 'upsert',
        type,
      ...
      if (!msg.key.fromMe) {
        // Unauthorised contacts are listener-only: inspect for the keyword,
        // but never enqueue them for Hermes or reply to them.
        listenOnly = !matchesAllowedUser(senderId, ALLOWED_USERS, SESSION_DIR);
        ...
        if (!listenOnly && WHATSAPP_DM_POLICY !== 'pairing' && !matchesAllowedUser(senderId, ALLOWED_USERS, SESSION_DIR)) {
          try {
            console.log(JSON.stringify({
              event: 'ignored',
      ...
      if (TEAM_MANAGER_OBSERVER.enabled) {
        TEAM_MANAGER_OBSERVER.observe({
          chatId,
          senderId,
          messageId: msg.key.id,
          text: event.body || '',
          ...
          fromMe: !!msg.key.fromMe,
          receivedAt: msg.messageTimestamp ? Number(msg.messageTimestamp) : null,
        });
      }
      await forwardKeywordMessage({ msg, chatId, senderId, body: event.body || '' });
      if (listenOnly) continue;
      ...
      // Security & role enforcement for team members / non-owners:
      const isSenderOwner = fromOwner || isOwnerSender(senderId, SESSION_DIR, HERMES_HOME);
      if (!isSenderOwner) {
        // Block slash commands (/new, /model, /yolo, /status, etc.) from team members
        if (event.body && event.body.trim().startsWith('/')) {
          const { content: payload, options } = buildTextSendPayload(
            'Perintah sistem tidak diizinkan. Silakan kirim laporan atau pertanyaan terkait operasional toko.',
            { chatId, messageStore }
          );
          sendWithTimeout(chatId, payload, options).catch(() => {});
          continue;
        }

        // Tag message clearly with employee identity so the agent knows who is speaking
        const memberInfo = getSenderMemberInfo(senderId, SESSION_DIR, HERMES_HOME);
        const memberTag = memberInfo
          ? `[LAPORAN KARYAWAN: ${memberInfo.name} (${memberInfo.role}) | ID: ${memberInfo.id}]`
          : `[LAPORAN ANGGOTA TIM]`;
        event.body = `${memberTag}\n${event.body}`;
      }
