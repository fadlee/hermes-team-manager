...
test('matchesAllowedUser accepts number directly registered in company/members files', () => {
  const root = mkdtempSync(path.join(os.tmpdir(), 'hermes-company-allowlist-'));
  const sessionDir = mkdtempSync(path.join(os.tmpdir(), 'hermes-wa-allowlist-'));

  try {
    const membersDir = path.join(root, 'company', 'members');
    mkdirSync(membersDir, { recursive: true });
    writeFileSync(path.join(membersDir, 'budi.md'), '---\nwhatsapp: "6285876047320"\nstatus: active\n---\n# Budi');

    // allowlist config is empty/only owner, but budi is in company/members
    const allowedUsers = parseAllowedUsers('6281227785005');
    assert.equal(matchesAllowedUser('6285876047320@s.whatsapp.net', allowedUsers, sessionDir, root), true);
    assert.equal(matchesAllowedUser('628999999999@s.whatsapp.net', allowedUsers, sessionDir, root), false);
  } finally {
